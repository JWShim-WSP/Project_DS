...
def post_save_update_product_info_from_Purchase(sender, instance, **kwargs):
    ...
    if instance.status == "Stocked":
        # let's increase the inventory for the product of instance (Purchase)
        instance.product.inventory += instance.quantity

        total_ex_rate_to_KRW = 0
        total_unit_price = 0

        # Let's get the average of latest 10 Purchase
        purchase_qs = Purchase.objects.filter(product=instance.product).order_by('-updated')[:10]
        loop_count = 0
        ...
        if loop_count: # do not divide by 0 in any case
            instance.product.average_ex_rate_to_KRW = total_ex_rate_to_KRW / loop_count
            instance.product.average_unit_price = total_unit_price / loop_count
            instance.product.average_unit_price_KRW = (total_unit_price * instance.product.average_ex_rate_to_KRW) / loop_count
        else:
